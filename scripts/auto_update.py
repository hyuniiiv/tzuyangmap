...
def llm_extract_restaurant(title: str, sub_text: str, desc_text: str,
    ...
    sub  = (sub_text or "")[:5000]    # 자막 활용 강화 (2200 → 5000)
    desc = (desc_text or "")[:700]
    com  = (comments_text or "")[:10000]  # 댓글 더 많이 (6000 → 10000, ~150개)
    web  = (web_snippets or "")[:2000]

    prompt = (
        "한국 음식 먹방 유튜브 영상에서 방문한 매장의 정확한 정보를 추출하세요.\n"
        "**정확도 0순위**. 모든 단서를 종합해 매장을 식별.\n\n"
        f"[영상 제목]\n{title}\n\n"
        f"[자막 — 쯔양/사장님 발화 (위치 단서의 핵심 소스)]\n{sub or '(없음)'}\n\n"
        f"[영상 설명]\n{desc or '(없음)'}\n\n"
        f"[댓글 — 좋아요/하트 우선 정렬, 최대 150개]\n{com or '(없음)'}\n\n"
        f"[웹 검색 결과 (영상 제목+쯔양으로 검색한 페이지)]\n{web or '(없음)'}\n\n"
        "[썸네일 이미지] — 별도 첨부 (간판/로고/외관 분석)\n\n"
        "## 분석 지침\n\n"
        "### 1. 자막에서 위치 단서 추출 (최고 신뢰도)\n"
        "다음 패턴을 자막에서 찾으세요:\n"
        "- 사장님 자기소개: '저희 가게는 OO에서 30년...', 'OO동에서 운영 중', 'OO 출신'\n"
        "- 쯔양 도착 멘트: '여기는 OO입니다', 'OO에 왔습니다', 'OO역 근처', 'OO 차 타고 왔어요'\n"
        "- 가게 자랑/유래: 'OO지점은 처음 생긴 곳', '본점이 OO에', '체인 1호점이 OO'\n"
        "- 메뉴 설명: 'OO식 OOO', 'OO지역 전통...', 'OO 특산물'\n"
        "- 손님/직원 대화: 'OO에서 자주 와요', 'OO쪽 분이세요?'\n"
        "- 일행 멘트: '여기 OO 처음 와봐', 'OO 유명한 데'\n"
        "→ 이런 단서가 있으면 confidence: high\n\n"
        "### 2. 댓글에서 지점/주소 정보 찾기\n"
        "- '저 식당 OO점 맞죠?' / '아 OO에 있는 그 가게!' / 답글에 '맞아요 OO점'\n"
        "- '주소가 어디예요?' 질문 → 답글에 도로명 주소\n"
        "- '나도 OO지점 가봤어요' / '집근처 OO점' / 'OO동에 새로 생겼더라'\n"
        "- '본점이 더 맛있다 vs OO점이 진짜다' 같은 지점 비교\n\n"
        "### 3. 썸네일 이미지 분석\n"
        "- 매장 외관: 간판 글자, 상호, 영업표시\n"
        "- 메뉴판/포스터: 메뉴명, 매장명, 분점 표시\n"
        "- 음식 패키지/포장 상표: 브랜드 식별\n\n"
        "### 4. 약칭/별명 정식 매장명으로 변환\n"
        "- '엽떡' → '동대문엽기떡볶이' (지점은 다른 단서로 파악)\n"
        "- '망리단길 빵집' → 구체적 매장명 (썸네일/댓글에서)\n"
        "- '본점' 단독 언급 → 그 브랜드 본점 (체인이면 ㅇㅇ '본점')\n\n"
        "### 5. 매장 단위 인정 범위\n"
        "다음은 정당한 restaurant_name 후보입니다 (큰 단위라도 OK):\n"
        "- 개별 매장 (예: '동대문엽기떡볶이 가락점', '곰탕한그릇')\n"
        "- **시장/푸드코트** (예: '노량진수산시장', '광장시장', '강릉중앙시장')\n"
        "- **음식 거리/골목** (예: '남대문갈치골목', '신당동떡볶이타운', '청계천막걸리거리')\n"
        "- **푸드트럭 거리/포차 거리** 도 OK\n"
        "→ 영상이 시장/거리 전체를 다루면 그 명칭 자체가 답이 될 수 있음\n\n"
        "### 6. 절대 금지\n"
        "- 메뉴명/일반명사 단독 (X: '로제떡볶이', '삼겹살집', '국밥집', '시골마을')\n"
        "- 추측으로 지점명 만들기 (단서 없이 '가락점' 임의 지정 금지)\n"
        "- 영상이 식당 방문이 아니면 (Q&A, 해명, 만남 영상) null\n"
        "- 확실하지 않으면 restaurant_name=null, brand만 채우기\n\n"
        "### 7. 주소 형식\n"
        "도로명 주소 (예: '서울 송파구 양재대로62길 16'). 시장/거리는 대표 주소 또는 행정동.\n\n"
        "### 8. confidence 기준\n"
